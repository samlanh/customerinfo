...
class Other_Model_DbTable_DbVillage extends Zend_Db_Table_Abstract
{
	...
	function getAllVillage($search=null){
		...
		$sql.=" ,(SELECT first_name FROM rms_users WHERE id=v.user_id LIMIT 1) AS user_name FROM ln_village AS v,`ln_commune` AS c, `ln_district` AS d , `ln_province` AS p
       ...
       public function getAllvillagebyCommune($village_id){
		$db = $this->getAdapter();
		$sql = "SELECT vill_id AS id,village_namekh AS name FROM $this->_name WHERE village_namekh!='' AND status=1 AND commune_id=".$db->quote($village_id);
