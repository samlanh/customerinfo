<?php
class Other_LineController extends Zend_Controller_Action {
	...
	public function indexAction(){
		try{
			...
			$link=array(
					'module'=>'other','controller'=>'line','action'=>'edit',
			);
   	...
   	if($this->getRequest()->isPost()){
   		try{
   			...
   			if(!empty($_data['save_new'])){
   				Application_Form_FrmMessage::Sucessfull($this->tr->translate('INSERT_SUCCESS'), self::REDIRECT_URL.'/line/add');
   			}else{
   				Application_Form_FrmMessage::Sucessfull($this->tr->translate('INSERT_SUCCESS'), self::REDIRECT_URL.'/line/index');
   			}
   		}catch(Exception $e){
   	...
   	$db = new Other_Model_DbTable_DbOther();
	   	if($this->getRequest()->isPost()){
	   		try{
	   			...
	   			$_data = $this->getRequest()->getPost();
	   			$db->addOther($_data,$this->typetable);
	   			Application_Form_FrmMessage::Sucessfull($this->tr->translate('EDIT_SUCCESS'),self::REDIRECT_URL.'/line/index');
	   		}catch(Exception $e){
	    ...
	    $row = $db->getOtherById($id,$this->typetable);
	   	if(empty($row)){
	   		Application_Form_FrmMessage::Sucessfull($this->tr->translate('NO_RECORD'),self::REDIRECT_URL.'/line/index');
	   		exit();
	   	}
	   	$frm = new Other_Form_FrmOther();
