<?php
class Other_SideController extends Zend_Controller_Action {
	...
	public function indexAction(){
		try{
			...
			$link=array(
					'module'=>'other','controller'=>'side','action'=>'edit',
			);
   	...
   	if($this->getRequest()->isPost()){
   		try{
   			...
   			if(!empty($_data['save_new'])){
   				Application_Form_FrmMessage::Sucessfull($this->tr->translate('INSERT_SUCCESS'), self::REDIRECT_URL.'/side/add');
   			}else{
   				Application_Form_FrmMessage::Sucessfull($this->tr->translate('INSERT_SUCCESS'), self::REDIRECT_URL.'/side/index');
   			}
   		}catch(Exception $e){
   	...
   	$db = new Other_Model_DbTable_DbOther();
	   	if($this->getRequest()->isPost()){
	   		try{
	   			...
	   			$_data = $this->getRequest()->getPost();
	   			$db->addOther($_data,$this->typetable);
	   			Application_Form_FrmMessage::Sucessfull($this->tr->translate('EDIT_SUCCESS'),self::REDIRECT_URL.'/side/index');
	   		}catch(Exception $e){
	   	...
	   	if(empty($row)){
	   		Application_Form_FrmMessage::Sucessfull($this->tr->translate('NO_RECORD'),self::REDIRECT_URL.'/side/index');
	   		exit();
	   	}
	   	$frm = new Other_Form_FrmOther();
